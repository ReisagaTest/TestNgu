...
local function initialize()
    ...
    local success, result = pcall(function()
        local Main = Client.PlayerGui:WaitForChild("Main", 10)
        local Loading = Main:WaitForChild("Loading", 10)
        local Version = Main:WaitForChild("Version", 10):WaitForChild("Script").Parent
        local start = tick()

        repeat task.wait() until Version.Text:find("Version")
        repeat
            task.wait()
    ...
    local env = getgenv()
    env.getconstant = debug.getconstant
    env.getconstants = debug.getconstants
    env.getupvalue = debug.getupvalue
    env.getupvalues = debug.getupvalues
    env.getgc = env.getgc or debug.getregistry

    return true
end

local function checkExploit(exploit)
    local success, response = pcall(function()
        local res = Request({
            Url = "https://httpbin.org/get",
            Method = "GET"
        }).Body
        local decode = HttpService:JSONDecode(res)
        return decode.headers['User-Agent'] == exploit
    end)
    return success and response or false
end

local function formatEnemyName(name)
...
local function setFPSCap()
    ...
    pcall(function()
        ...
        if _G.Settings.Other["FPS Cap"] then
            local cap = _G.Settings.Other["FPS Cap"] or "60 FPS"
...
Toggle = Volcanic:AddToggle("Toggle", {
    Title = "Auto Use Skill Fruit",
    Default = false
})
Toggle:OnChanged(function(v405)
    getgenv().UseFruit = v405
end)
...
local function Stack(Farm)
    ...
    elseif Farm == "Raid Pirate" then
        if not VerifyRaidPirate() then
            if getgenv().AutoFarmBone then return true
            elseif getgenv().AutoFarm_Level then return true
            elseif getgenv().AutoEliteHunter then return true
            elseif getgenv().TeleportToFruit then return true
            end
        end
    elseif Farm == "All Bosses" then
        if getgenv().KillAllBosses and VerifyBosses() then return true end
    elseif Farm == "Elite Hunter" then
        if getgenv().TeleportToFruit and FruitFind() then return true
        elseif VerifyRaidPirate() then return true
        elseif not VerifyEliteBoss() then
            if getgenv().AutoCandy then return true
            elseif getgenv().AutoFarm_Level then return true
            elseif getgenv().AutoFarmBone then return true
            end
        elseif VerifyNPC("rip_indra True Form") and getgenv().AutoRipIndra then return true
        elseif VerifyNPC("Cake Prince") and getgenv().AutoCakePrince then return true
        elseif VerifyNPC("Dough King") and getgenv().AutoCakePrince then return true
    ...
    elseif Farm == "Hallow Boss" then
        ...
        elseif getgenv().AutoRipIndra and VerifyNPC("rip_indra True Form") then return true
        end
    ...
    elseif Farm == "Don Swan" then
        if not VerifyNPC("Don Swan") then
            if getgenv().AutoFarm_Level then return true
            elseif getgenv().AutoFarmEctoplasm then return true
            end
    ...
    elseif Farm == "Rip Indra" then
        if not VerifyTool("God's Chalice") and not VerifyNPC("rip_indra True Form") then
            if getgenv().AutoFarm_Level then return true
            elseif getgenv().RipIndraLegendaryHaki and GetButton() then return true
            elseif getgenv().AutoEliteHunter then return true
            elseif getgenv().TeleportToFruit then return true
            elseif getgenv().AutoFarmBone then return true
            elseif getgenv().AutoCakePrince then return true
            end
        end
    end
    return false
...
function TaskHandler:GetRunningTasks(config)
    ...
    local runningTasks = {}
    for name, task in pairs(ListTasks) do
        if task:IsRunning() and not whitelist[name] then
